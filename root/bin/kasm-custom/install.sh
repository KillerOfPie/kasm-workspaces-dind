...
function set_random_guac_token() {
    # Honor the default if its passed in
    if [ "${DEFAULT_GUAC_TOKEN}" == "false" ] && [ "${ROLE}" != "guac" ]; then
        DEFAULT_GUAC_TOKEN="$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 22)"
    elif [ "${DEFAULT_GUAC_TOKEN}" == "false" ] && [ "${ROLE}" == "guac" ]; then
...
function configure_guac_component() {
    if [ "${ROLE}" == "guac" ]; then
        if [ "${API_SERVER_HOSTNAME}" != "false" ] && [ "${PUBLIC_HOSTNAME}" != "false" ] && [ "${REGISTRATION_TOKEN}" != "false" ]; then
            FILE=${KASM_INSTALL_BASE}/conf/app/kasmguac.app.config.yaml
            if [ -f "${FILE}" ]; then
...
function base_install() {
    chmod +x -R ${KASM_INSTALL_BASE}/bin
    chmod 750 -R ${KASM_INSTALL_BASE}/bin
...
function connectivity_test() {
    if [ "${ROLE}" == "app" ]; then
        ...
        for i in {1..10}; do
            ...
            if [[ "${DB_CHECK}" =~ .*"succeeded".* ]] || [[ "${DB_CHECK}" =~ .*"Connected".* ]]; then
        ...
        for i in {1..10}; do
            ...
            if [[ "${REDIS_CHECK}" =~ .*"succeeded".* ]] || [[ "${REDIS_CHECK}" =~ .*"Connected".* ]]; then
                REDIS_CONNECTED="true"
                break
            fi
            sleep 3
        done
        if [ "${REDIS_CONNECTED}" == "true" ] && [ "${POSTGRES_CONNECTED}" == "true" ]; then
...
function check_role() {
    ...
    if [ "${ROLE}" != "all" ] && [ "${ROLE}" != "agent" ] && [ "${ROLE}" != "app" ] && [ "${ROLE}" != "db" ] && [ "${ROLE}" != "init_remote_db" ] && [ "${ROLE}" != "guac" ] && [ "${ROLE}" != "proxy" ]; then
